      ...
      <main className="px-4">
        {viewMode === "list" ? (
          <div className="divide-y">
            {catOwners.map((owner) => (
              <Link key={owner.id} href={`/sitter/connect/${owner.id}`}>
                <div className="py-4 flex items-center gap-4 hover:bg-muted/50 transition-colors -mx-4 px-4">
          ...
          <div className="relative w-full h-[calc(100vh-240px)] -mx-4">
            ...
            <div className="absolute inset-0 pointer-events-none">
              {catOwners.map((owner, index) => (
                <Link key={owner.id} href={`/sitter/connect/${owner.id}`} className="pointer-events-auto">
                  <div
